...
@Component({
  selector: 'app-dashmonitor',
  imports: [Titulo, Path, CommonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './dashmonitor.html',
  styleUrl: './dashmonitor.scss',
  standalone: true
})
...
@ViewChild('btnModal') btnModal:any;
  ...
  alto:string =  ((screen.height / 2)-25).toString();
  ...
  sendMonitor() {
    if (this.serverIndex == this.lstServidores.length) {
      ...
      this.continuar = false;
      if(this.lstServidores[this.serverIndex].check){
